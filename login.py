...
from ui.login_gui import Ui_MainWindow
...
class LOGIN(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.bound.setEditable(True) # 需要先设置可编辑
        self.bound.lineEdit().setAlignment(QtCore.Qt.AlignCenter) # 再设置居中方式
        ...
        try:
            ...
            self.data = []
        self.label_pix.setPixmap(QPixmap("img/logo.png"))
    ...
    def init(self):
        ...
        self.com.currentTextChanged.connect(self.port_imf)
        self.save_button.clicked.connect(self.com_save)
    ...
    def port_imf(self):
        # 显示选定的串口的详细信息
        imf_s = self.com.currentText()
        if imf_s != "":
            self.check.setText(self.Com_Dict[self.com.currentText()])

    def com_save(self):
        if os.path.exists(r'port.txt'):
            os.remove(r'port.txt')
        with open('port.txt', 'a', encoding='utf8') as f:  # 写入
            f.writelines(self.com.currentText() + '\n')
